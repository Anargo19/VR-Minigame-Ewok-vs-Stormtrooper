    ...
    public class NetworkManager : MonoBehaviourPunCallbacks
    {
        ...
        void Start()
        {
            ...
            else
            {
                ...
                if (PlayerNetwork.UserMeInstance == null)
                {
                    Debug.LogFormat("We are Instantiating LocalPlayer from {0}", SceneManagerHelper.ActiveSceneName);
                    // we're in a room. spawn a character for the local player. it gets synced by using PhotonNetwork.Instantiate
                    // PhotonNetwork.Instantiate("Prefabs/" + playerPrefabPC.name, new Vector3(0f, 5f, 0f), Quaternion.identity);
                    GameObject avatar = UserDeviceManager.GetPrefabToSpawnWithDeviceUsed(playerPrefabPC, playerPrefabVR);
                    PhotonNetwork.Instantiate("Prefabs/" + avatar.name, new Vector3(92.2f, 34.56f, 5.67f), Quaternion.identity);
                    if(UserDeviceManager.GetDeviceUsed() != UserDeviceType.PC)
                        Instantiate(teleportArea, new Vector3(92.2f, 34.56f, 5.67f), Quaternion.identity);
